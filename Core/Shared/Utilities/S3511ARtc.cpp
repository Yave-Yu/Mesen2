...
void S3511ARtc::LoadBattery(string console)
{
	vector<uint8_t> rtcData = _emu->GetBatteryManager()->LoadBattery(console, ".rtc");
...
void S3511ARtc::SaveBattery(string console)
{
	...
	_emu->GetBatteryManager()->SaveBattery(console, ".rtc", rtcData.data(), (uint32_t)rtcData.size());
}
